	...
	public double[] posAfterMove(String move) {
		...
		if (direction == 90) {
			if (m == RobotConstants.FORWARD)
				...
				if (isRealBot) {
					p[0] += RobotConstants.LEFT_F_V;
					p[1] -= RobotConstants.LEFT_F_H;
				} else {
					p[0] += radius;
					p[1] -= radius;
				}
				...
				if (isRealBot) {
					p[0] += RobotConstants.RIGHT_F_V;
					p[1] += RobotConstants.RIGHT_F_H;
				} else {
					p[0] += radius;
					p[1] += radius;
				}
				...
				if (isRealBot) {
					p[0] -= RobotConstants.LEFT_B_V;
					p[1] -= RobotConstants.LEFT_B_H;
				} else {
					p[0] -= radius;
					p[1] -= radius;
				}
				...
				if (isRealBot) {
					p[0] -= RobotConstants.RIGHT_B_V;
					p[1] += RobotConstants.RIGHT_B_H;
				} else {
					p[0] -= radius;
					p[1] += radius;
				}
		...
		// east
			...
			if (m == RobotConstants.FORWARD)
				...
				if (isRealBot) {
					p[0] += RobotConstants.LEFT_F_H;
					p[1] += RobotConstants.LEFT_F_V;
				} else {
					p[0] += radius;
					p[1] += radius;
				}
				...
				if (isRealBot) {
					p[0] -= RobotConstants.RIGHT_F_H;
					p[1] += RobotConstants.RIGHT_F_V;
				} else {
					p[0] -= radius;
					p[1] += radius;
				}
				...
				if (isRealBot) {
					p[0] += RobotConstants.LEFT_B_H;
					p[1] -= RobotConstants.LEFT_B_V;
				} else {
					p[0] += radius;
					p[1] -= radius;
				}
				...
				if (isRealBot) {
					p[0] -= RobotConstants.RIGHT_B_H;
					p[1] -= RobotConstants.RIGHT_B_V;
				} else {
					p[0] -= radius;
					p[1] -= radius;
				}
