...
public class Simulator {
    ...
    private static void initButtonsLayout() {
        _buttons.setLayout(new GridLayout(2,7));
        /*
         * LoadMap	|| Reset position	|| Cells to arrive	|| Hamiltonian Path	|| Fastest Path	|| Connect RPI		|| Disconnect RPI
         * Forward	|| Backward			|| Left Forward		|| Right Forward	|| Left Backward|| Right Backward	|| Take Picture (TP)
         */
        addButtons();
    }
    ...
    private static void addButtons() {
        ...
        // HPath Class for Multithreading
        ...
        btn_ConnectRPI.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
            	...
            	if (a==1) {	// can connect
            		...
            		// loadMapFromDisk(map, "newmap14");
            		// String aaaa = comm.recvMsg();
            		CardLayout cl = ((CardLayout) _mapCards.getLayout());
                    cl.show(_mapCards, "REAL_MAP");
                    
            		loadMapFromAndroid(map, bot);
                    // loadMapFromDisk(map, "newmap13");
        ...
        // Disconnect RPI
        JButton btn_DisconnectRPI= new JButton("Disconnect RPI");
        formatButton(btn_DisconnectRPI);
        btn_DisconnectRPI.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
            	if (comm != null)
            		comm.closeConnection();
            }
        });
        _buttons.add(btn_DisconnectRPI);
        ...
        // Take Picture
        JButton btn_TakePic= new JButton("Take Pic (TP)");
        formatButton(btn_TakePic);
        btn_TakePic.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
            	if (comm != null) {
            		comm.sendMsg("TP__1", comm.toRPI);
            		// comm.recvMsg();
            	}
            }
        });
        _buttons.add(btn_TakePic);
    }
    ...
    public static int takePic() {
    	...
    	if (bot.getRealBot()) {
			comm.sendMsg("TP__1", CommMgr.toRPI);
