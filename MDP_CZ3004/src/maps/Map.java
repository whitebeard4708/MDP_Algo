...
public class Map extends JPanel{
	...
	private ArrayList<Cell> obstacles = new ArrayList<Cell>();
	...
	private ArrayList<double[]> positionToArrive = new ArrayList<double[]>();
    ...
    public double[] getPositionToArrive(int index) {
    	return this.positionToArrive.get(index);
    }
    ...
    public void setObsId(int row, int col, String obs_id) {
    	grid[row][col].setObsId(obs_id);
    }
    ...
    public void findPositionToArrive() {
    	...
    	for (int i=0; i<numObstacleFound; i++) {
    		...
    		int image_side = obstacles.get(i).getImageSide();
    		double r = obstacles.get(i).getRow();
    		double c = obstacles.get(i).getCol();
    		String idd = obstacles.get(i).getObsId().substring(1);
    		double obs_id = Double.parseDouble(idd);
    		
    		// skip unaccessible images
    		if (image_side == 90 && r >= MapConstants.MAP_ROW - 6) continue;
    		else if (image_side == 0 && c >= MapConstants.MAP_COL - 6) continue;
    		else if (image_side == -90 && r < 6) continue;
    		else if (image_side == 180 && c < 6) continue;
    		...
    		// add new cell
    		// check valid
    		if (checkValidRobotCenter(cellToArriveRow+.5, cellToArriveCol+.5)) {
    			double [] a = new double[4];
    			a[0] = cellToArriveRow;
				a[1] = cellToArriveCol;
				a[2] = cellToArriveDirection;
				a[3] = obs_id;
				positionToArrive.add(a);
				System.out.println("Add new cells to arrive: (O" + obs_id + ", " + cellToArriveRow + ", " + cellToArriveCol + ", " + cellToArriveDirection + ")");
    		}
    	}
    	isDrawPositionToArrive = true;
    	System.out.println("Total " + positionToArrive.size() + " places to capture images.");
    }
    
    
    public int getNumPositions() {
    	return this.positionToArrive.size();
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int mapRow = 0; mapRow < MapConstants.MAP_ROW; mapRow++) {
        	...
        	g.drawString(Integer.toString(mapRow), 80, 620 - mapRow * GraphicsConstants.CELL_SIZE);
        	
            for (int mapCol = 0; mapCol < MapConstants.MAP_COL; mapCol++) {
            	g.setColor(Color.BLACK);
            	g.drawString(Integer.toString(mapCol), 130 + mapCol * GraphicsConstants.CELL_SIZE, 650);
                Color cellColor;
        ...
        if (isDrawPositionToArrive) {
        	...
        	for (int i=0; i<positionToArrive.size(); i++) {
        		double[] position = positionToArrive.get(i);
        		int crow = (int) Math.floor(position[0]);
        		int ccol = (int) Math.floor(position[1]);
