...
public class Storage {
	...
	// for fastest path
	private static int[] fastest_order;
	private static String[] optimalPath;
	
	// for Hamiltonian path
	private static int[] good_order;
	...
	private Storage(Map map, Robot bot) {
		this.map = map;
		this.bot = bot;
		n_position = map.getNumPositions();
		fastest_order = new int[n_position];
		good_order = new int[n_position];
		for (int i=1; i<= n_position; i++) {
			fastest_order[i-1] = i;
			good_order[i-1] = i;
		}
	}
	...
	public static String[] getFastestPath() {
		...
		optimalPath = new String[n_position];
	...
	public static String[] getGoodPath() {
		...
		goodPath = new String[n_position];
	...
	public static void initValues() {
		...
		nodes = new Node[n_position+1]; // starting node also count
		pathCost = new double [n_position+1][n_position+1];
		paths = new String[n_position+1][n_position+1];
	...
	public static String robotPath(String sim_path) {
		if (sim_path.equals("")) return "";
		
		String rpath = "";
		String[] steps = sim_path.split("/");
		double currentd = 0;
		for (String step: steps) {
			char m = step.charAt(0);
			if (m == RobotConstants.FORWARD || m == RobotConstants.BACKWARD) {
				currentd = currentd + distance(step);
			} else {
				String[] rsteps = convertStep(step);
				currentd = currentd + distance(rsteps[0]);
				// System.out.println("aaa "  + currentd);
				if (currentd > 0) {
					double ddd = currentd;
					while (ddd > 4) {
						rpath = rpath + "/" + String.format("%c030", RobotConstants.FORWARD);
						ddd = ddd - 3;
					}
					rpath = rpath + "/" + String.format("%c%03d", RobotConstants.FORWARD, (int) (ddd * 10));
				} else if (currentd < 0) {
					double ddd = currentd;
					while (ddd < -4) {
						rpath = rpath + "/" + String.format("%c030", RobotConstants.BACKWARD);
						ddd = ddd + 3;
					}
					rpath = rpath + "/" + String.format("%c%03d", RobotConstants.BACKWARD, (int) (-ddd * 10));
				}
				rpath = rpath + "/" + rsteps[1];
				currentd = distance(rsteps[2]);
			}
		...
		currentd = 0;
		return rpath.substring(1);
	}
	
	private static double distance(String step) {
		double d = 0;
		char m = step.charAt(0);
		if (m == RobotConstants.FORWARD)
			d = Integer.parseInt(step.substring(1)) * 1.0;
		else if (m == RobotConstants.BACKWARD)
			d = - Integer.parseInt(step.substring(1)) * 1.0;
		return d / 10;
	}
	
	public static String[] convertStep(String step) {
		char m = step.charAt(0);
		String[] rsteps = new String[3];
		rsteps[1] = step;
		double vdif, hdif;
		String w1, w2;
		
		if (m == RobotConstants.LEFT_FORWARD) {
			vdif = RobotConstants.FAKE_RADIUS - RobotConstants.LEFT_F_V;
			hdif = RobotConstants.FAKE_RADIUS - RobotConstants.LEFT_F_H;
			if (vdif >= 0) {
				w1 = String.format("%c%03d", RobotConstants.FORWARD, (int) (vdif*10));
			} else {
				w1 = String.format("%c%03d", RobotConstants.BACKWARD, (int) (-vdif*10));
			}
			
			if (hdif >= 0) {
				w2 = String.format("%c%03d", RobotConstants.FORWARD, (int) (hdif*10));
			} else {
				w2 = String.format("%c%03d", RobotConstants.BACKWARD, (int) (-hdif*10));
			}
		} else if (m == RobotConstants.RIGHT_FORWARD) {
			vdif = RobotConstants.FAKE_RADIUS - RobotConstants.RIGHT_F_V;
			hdif = RobotConstants.FAKE_RADIUS - RobotConstants.RIGHT_F_H;
			if (vdif >= 0) {
				w1 = String.format("%c%03d", RobotConstants.FORWARD, (int) (vdif*10));
			} else {
				w1 = String.format("%c%03d", RobotConstants.BACKWARD, (int) (-vdif*10));
			...
			vdif = RobotConstants.FAKE_RADIUS - RobotConstants.LEFT_B_V;
			hdif = RobotConstants.FAKE_RADIUS - RobotConstants.LEFT_B_H;
			if (vdif >= 0) {
				w1 = String.format("%c%03d", RobotConstants.BACKWARD, (int) (vdif*10));
			} else {
				w1 = String.format("%c%03d", RobotConstants.FORWARD, (int) (-vdif*10));
			...
			vdif = RobotConstants.FAKE_RADIUS - RobotConstants.RIGHT_B_V;
			hdif = RobotConstants.FAKE_RADIUS - RobotConstants.RIGHT_B_H;
			if (vdif >= 0) {
				w1 = String.format("%c%03d", RobotConstants.BACKWARD, (int) (vdif*10));
			} else {
				w1 = String.format("%c%03d", RobotConstants.FORWARD, (int) (-vdif*10));
			...
			w1 = String.format("%c%03d", RobotConstants.FORWARD, 0);
			w2 = String.format("%c%03d", RobotConstants.FORWARD, 0);
		}
		
		rsteps[0] = w1;
		rsteps[2] = w2;
		// System.out.println(Arrays.toString(rsteps));
		return rsteps;
